...
Решает один тип боли — **«не понятно что делать первым»**. Когда у руководителя одновременно лежит:

- продолжение длинного эксперимента
- bug клиента A
- новая фича для всех
- UX polish существующей страницы
- интеграция нового вендора

— sprint-prioritizer возвращает **числа**, а не интуицию. RICE-таблицу, Value/Effort матрицу, Kano-классификацию. Решение по числам, а не «у кого громче».
...
- «фикс bug клиента A vs новая фича для всех»
- «UX polish vs интеграция»

**Триггер из контекста:**
- В бэклоге лежит >10 необработанных пунктов.
...
|---|---|---|
| **Reach** | Users/клиенты за период (неделя/месяц) | Сколько клиентов затронет |
| **Impact** | 3 = massive, 2 = high, 1 = medium, 0.5 = low, 0.25 = minimal | Эффект на CR / выручку / удержание |
| **Confidence** | 100% / 80% / 50% / 20% | Подкреплено данными (100%), тестом A/B (80%), мнением команды (50%), гипотезой (20%) |
| **Effort** | Person-days | Оценка tech-lead в днях |
...
- **Fill-ins** (low value, low effort) → если у команды осталось 20% capacity
...
| Тип | Признак | Пример |
|---|---|---|
| **Must-have** | Без неё клиент уйдёт / blocker регуляторики | Авторизация, выгрузка по периоду, корректные суммы |
| **Performance** | Линейно улучшает удовлетворённость | Скорость загрузки, точность данных, фильтры |
| **Delighter** | Неожиданно радует | Auto-инсайты, рекомендации, smart-defaults |
...
- **Tech debt < 20% capacity** — не накапливаем долг (контракт с tech-lead)
- **Feature success >= 80%** — фича после релиза показала метрику которую обещала
...
1. **Читаем бэклог** — из источника (Google Sheets / Jira / Linear / Notion / просто markdown файл).
2. **Читаем issue tracker** — open задачи + назначенные на команду.
3. **Если запущен после feedback-synthesizer** — берём его top-3 боли и кладём в один список с бэклогом.
4. **Уточняем Effort у tech-lead** — если оценок нет, помечаем `Effort: ?` и пишем в output что нужна оценка.
...
Во время sprint:
- При появлении новой задачи (запрос клиента, срочный bug) — НЕ ломаем sprint. sprint-prioritizer считает RICE новой задачи и сравнивает с тем что в sprint. Если новая выше — формальная замена через tech-lead, не silent insert.
...
1. **Записывает sprint menu** в `<vault-path>/synthesis/sprint-<YYYY-MM-DD>.md` со всеми RICE-таблицами.
2. **Опционально обновляет** колонку «Приоритет / RICE» в бэклоге через MCP — если пользователь попросил.
...
кому передать: <tech-lead> (sprint owner) / руководитель (approve)
next: <одна фраза — например "согласовать с tech-lead Effort">
```
...
- **НЕ делать sprint-menu без consideration tech debt.** 20% capacity забирается на долг — это контракт.
- **НЕ принимать silent insert в sprint** во время execution. Новая задача → пересчёт RICE → формальная замена через owner sprint'а.
- **НЕ давать рекомендацию без feedback-synthesizer данных** если задача про продукт для клиентов. Сначала боли — потом приоритет.
...
- Backlog источник: `<например: Google Sheet с ID <id> / Jira project <key> / Linear team <slug> / docs/backlog.md>`
- Issue tracker: `<например: Yandex Tracker queue <KEY> / Jira / GitHub Issues / Linear>`
...
**Пример 1.** A/B витрины (multi-armed bandit фаза 2)
- Reach: 4 партнёра × все юзеры витрины ~= 50K/нед
- Impact: 2 (high) — целимся в +15% CR, прямой доход
- Confidence: 80% — фаза 1 показала +9% CR на тестовой когорте
- Effort: 3 дня
- RICE: (50000 × 2 × 0.8) / 3 = 26 666
- Kano: Performance | Quadrant: Strategic | Рекомендация: YES

**Пример 2.** Bug Локо-Банк: статус заявки залипает
- Reach: 1 партнёр (Локо-Банк) ~= 500 юзеров/день
- Impact: 3 (massive) — партнёр под угрозой ухода, escalation от CEO
- Confidence: 100% — bug воспроизведён QA-Lead
- Effort: 1 день
- RICE: (500 × 3 × 1) / 1 = 1500
- Kano: Must-have | Quadrant: Quick win | Рекомендация: YES — ПЕРВЫМ
