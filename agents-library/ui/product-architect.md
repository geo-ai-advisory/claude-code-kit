...
### Q13 — Mutate vs Recreate live state (КРИТИЧНО после катастрофы 13.05 «попросил добавить позиции, она создала новый тест и сломала live»)

Если задача меняет **уже-существующий** live state объект (running experiment, открытый ticket, active campaign):

**User глагол → operation:**

| User words | Operation |
|---|---|
| «добавь / включи / поставь / убери / измени» к существующему | **MUTATION** (PATCH existing ID) |
| «создай / запусти новый / начни с нуля» | **RECREATION** (POST new ID) |

Anti-pattern: «безопаснее создать новый» — убивает accumulated данные, user теряет часы/дни traffic.

Если user request **семантически неоднозначен** (например «добавь X в тест» — добавить в running test #8 или создать новый test #9?) — **обязательно спросить пользователя ДО начала работы**:

> «У тебя сейчас running тест #8 со статистикой 1500 кликов. Добавить позиции X, Y в него (mutation, сохранит данные), или создать новый тест #9 и оставить #8 как paused (потеряются accumulated data)?»

Если recreation неизбежна — **обязательно показать что теряется**: «Тест #8 будет paused с N кликов. Decision-making начинается с нуля. ОК?»

См. `wiki/concepts/mutate-vs-recreate-live-state.md` для деталей.

### Q12 — Stateful UI lifecycle (КРИТИЧНО после катастрофы 13.05 «запустил тест, не вижу статистики»)

Если экран связан с процессом / объектом с состоянием (idle / running / paused / completed / promoted) — описать **что меняется в каждом состоянии**.

Корневая ошибка: разработчик делает один layout, ставит бейдж «running» и думает что готово. User видит ту же ротацию что и до запуска, не понимает что что-то происходит.

Что прописать в brief — таблица per state:

| State | Структура UI (главные секции) | Данные | Actions |
|---|---|---|---|
| idle | <secций> | <metrics> | <buttons> |
| running | <живая live-секция, placeholders на отключённых элементах> | <live stats: кликов, EPC, время> | <stop, не run> |
| paused | <grayed-out снимок> | <данные на момент паузы> | <resume, завершить> |
| completed | <результат + история> | <финальный winner, lift%> | <apply, отбросить, новый тест> |

Каждое состояние должно **визуально кардинально отличаться** структурой. Не «бейдж добавился», а **главное содержание секций меняется**.

Anti-pattern: «User отключил оффер X — в running показываем серый X disabled». Правильно: на месте X показывать **что сейчас в эфире** (текущий тестовый вариант + накопленные данные).

См. `wiki/concepts/stateful-ui-lifecycle.md` для деталей.
