...
Для каждого компонента в screen-spec описать что происходит **после** каждого user action в виде YAML. Это критический input для qa-scenario-tester: без functional behavior он проверит только «элемент кликается» и пропустит functional bugs.

Формат — YAML с полями `on`, `result`, `side_effects`, `invariants`, `must_NOT_happen`:

```yaml
component: PartnerSwitcher
actions:
  - on: click trigger
  ...
  - on: type in search (5 keystrokes "мтсба")
  ...
  - on: click item (партнёр X)
    result: state.selectedPartner = X.key; localStorage.setItem('selected_partner', X.key); ВСЕ data sections страницы перерисовываются с partnerKey=X.key
    side_effects:
      - emit event 'partner-changed' для других компонентов (heatmap, leaderboard, kpi-cards)
      - close panel
      - update header partner-name
      - update URL `?partner=<X.key>` (если используется routing)
    invariants:
      - все секции (header + heatmap + table + cards) показывают данные X
      - network request содержит partnerId=X (verify через browser_network_requests)
      - body data-partner === X
    must_NOT_happen:
      - старая data партнёра предыдущего остаётся на heatmap / cards / table (AP-1 sync)
      - другие partner-dependent secondary elements (filters, badges) не обновлены (AP-11 cross-component)
      - двойной partner-switcher в DOM (legacy `<select>` + новый компонент) — split brain (AP-10)

  - on: reload page after partner=X selected
    result: page restores partner=X from URL > localStorage > default
    invariants:
      - URL.searchParams.partner === X (preserved)
      - localStorage.selected_partner === X
      - header показывает X
      - все секции грузятся с X data
    must_NOT_happen:
      - sсбрасывается на default partner после reload (AP-8 persistence)
      - URL ?partner=X игнорируется при initialization (P1 fix from 12.05)
...
actions:
  - on: click on variant Y row
    ...
    invariants:
      ...
      - другие varианты продолжают идти (traffic split не остановлен)
    must_NOT_happen:
  ...
  - on: click on variant Y with 5 clicks (edge data)
...
Если одна entity рендерится в нескольких компонентах одновременно (например star «базовый» + полный list порядка офферов) — обязательно описать **cross-component invariants**:
...
cross_components:
  - entity: current_base_order
    rendered_in:
      ...
      - OrderInlineList (inline список «1→2→3→...→7» рядом со звездой)
...
АВТОНОМНО (без approve пользователя на screen-spec):
...
6. Push только после явного «ок пуш» от пользователя
```

### Output в чат — ровно 5 строк
...
next: Edit → ui-quality-reviewer → qa-scenario-tester
```

**НЕ запрашивать approve на screen-spec.** Screen-spec — технический артефакт для main session, не product decision. Main session после ui-design-architect сразу делает Edit на HTML/CSS, не дёргая пользователя. Approve — только на push в prod.
